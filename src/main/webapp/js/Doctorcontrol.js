...
scotchApp.controller('doctorSearch',function($scope, $http){
    ...
    $scope.searchDoctor = function(loginDetail){
           ...
           if(loginDetail != null){
                  ...
                  doctorSearch = $http.post('https://doctor-service.cfapps.io/doctor/get/all', loginDetail);
                  $scope.loader = true;
           } 
...
scotchApp.controller('updateProfile',function($scope, $rootScope, $http, $cookieStore){
	$scope.doctors = $cookieStore.get('loginData');
	...
	$scope.doBlurMobile = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.mobile != null && $scope.doctors.mobile.length == 10){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurDegree = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.highestDegree != null && $scope.doctors.highestDegree.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurHomeAddress = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.homeAddress != null && $scope.doctors.homeAddress.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurExpertise = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.expertized != null && $scope.doctors.expertized.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurShopAddress = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.clinicAddress != null && $scope.doctors.clinicAddress.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurFees = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.oneTimeFee != null && $scope.doctors.oneTimeFee.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
	...
	$scope.doBlurConsulting = function($event){
		var target = $event.target;
		if($scope.doctors != null && $scope.doctors.daysCheckFree != null && $scope.doctors.daysCheckFree.length > 0){
			target.blur();	
		}else{
			target.focus();
		}
